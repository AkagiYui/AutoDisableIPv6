...
> ⚠️ 当前版本对「执行成功」的判定较为简单，使用时请知悉。

应用**仅依据 `su -c` 命令的退出码（exit code 为 0）** 来判定成功，**不会回读 `/proc` 的值，也不会检查实际的 IPv6 状态**。命令的原始输出只是原样记录在日志中供你查看，程序并不解析它。

由于上面的脚本是两条管道命令、且未启用 `set -e`，整段脚本的退出码取**最后一条命令**（写入 `all/disable_ipv6` 的 `tee`）。因此：

- 若第一条（针对具体网卡的 `accept_ra`，例如网卡名探测有误）失败、而第二条成功，仍会被记为**成功**，第一条的报错只体现在日志的输出文本里；
- 无 Root 或授权被拒时记为**失败**。

如需确认是否真正生效，请以「日志」页中的命令原始输出为准，或自行执行 `cat /proc/sys/net/ipv6/conf/all/disable_ipv6` 核对（结果应为 `1`）。
